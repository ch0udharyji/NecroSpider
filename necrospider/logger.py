...
from necrospider import NecroSpiderDb, NecroSpiderHelpers


class NecroSpiderSqliteLogHandler(logging.Handler):
    ...
    def makeDbh(self) -> None:
        """TBD."""
        self.dbh = NecroSpiderDb(self.opts)


def logListenerSetup(loggingQueue, opts: dict = None) -> 'logging.handlers.QueueListener':
    """Create and start a NecroSpider log listener in its own thread.
    ...
    log_dir = NecroSpiderHelpers.logPath()
    debug_handler = logging.handlers.TimedRotatingFileHandler(
        f"{log_dir}/necrospider.debug.log",
        when="d",
        interval=1,
        backupCount=30
    )
    ...
    error_handler = logging.handlers.TimedRotatingFileHandler(
        f"{log_dir}/necrospider.error.log",
        when="d",
        interval=1,
        backupCount=30
    )
    ...
    if doLogging and opts is not None:
        sqlite_handler = NecroSpiderSqliteLogHandler(opts)
...
def logWorkerSetup(loggingQueue) -> 'logging.Logger':
    ...
    """
    log = logging.getLogger("necrospider")
