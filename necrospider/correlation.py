...
class NecroSpiderCorrelator:
    """NecroSpider correlation capabilities.

    Todo:
        Make the rule checking per analysis method
    """

    log = logging.getLogger("necrospider.correlator")
    dbh = None
    scanId = None
    types = None
    rules = list()
    ...
    def __init__(self, dbh: NecroSpiderDb, ruleset: dict, scanId: str = None) -> None:
        """Initialize NecroSpider correlator engine with scan ID and ruleset.
        ...
        if not isinstance(dbh, NecroSpiderDb):
            raise TypeError(f"dbh is {type(dbh)}; expected NecroSpiderDb()")
    ...
    def build_db_criteria(self, matchrule: dict) -> dict:
        ...
        Returns:
            dict: criteria to be used with NecroSpiderDb.scanResultEvent()
