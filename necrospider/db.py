...
class NecroSpiderDb:
    ...
    # Queries for creating the NecroSpider database
    createSchemaQueries = [
    ...
    eventDetails = [
        ['ROOT', 'Internal NecroSpider Root event', 1, 'INTERNAL'],
        ...
        with self.dbhLock:
            try:
                ...
                try:
                    ...
                    raise IOError("Tried to set up the NecroSpider database schema, but failed") from e
            ...
            try:
                ...
                try:
                    ...
                    raise IOError("Looks like you are running a pre-4.0 database. Unfortunately "
                                  "NecroSpider wasn't able to migrate you, so you'll need to delete "
                                  "your NecroSpider database in order to proceed.") from None
    ...
    def scanEventStore(self, instanceId: str, sfEvent, truncateSize: int = 0) -> None:
        ...
        Args:
            instanceId (str): scan instance ID
            sfEvent (NecroSpiderEvent): event to be stored in the database
        ...
        if not isinstance(sfEvent, NecroSpiderEvent):
            raise TypeError(f"sfEvent is {type(sfEvent)}; expected NecroSpiderEvent()") from None
        ...
        if not isinstance(sfEvent.sourceEvent, NecroSpiderEvent) and sfEvent.eventType != "ROOT":
