...
from .threadpool import NecroSpiderThreadPool
...
class NecroSpiderPluginLogger(logging.Logger):
    """Used only in NecroSpiderPlugin to prevent modules
...
class NecroSpiderPlugin():
    """NecroSpiderPlugin module object
    ...
    Attributes:
        ...
        _currentEvent (NecroSpiderEvent): Current event being processed
    ...
    __scanId__ = None
    # (only used in NecroSpider HX) tracking of data sources
    __dataSource__ = None
    ...
    # NecroSpider object, set in each module's setup() function
    sf = None
    ...
    def log(self):
        if self._log is None:
            logging.setLoggerClass(NecroSpiderPluginLogger)  # temporarily set logger class
            self._log = logging.getLogger(f"necrospider.{self.__name__}")  # init NecroSpiderPluginLogger
    ...
    def setup(self, sf, userOpts: dict = {}) -> None:
        ...
        Args:
            sf (NecroSpider): NecroSpider object
            userOpts (dict): TBD
        """
        pass
    ...
    def setTarget(self, target) -> None:
        ...
        Args:
            target (NecroSpiderTarget): target

        Raises:
            TypeError: target argument was invalid type
        """
        from necrospider import NecroSpiderTarget

        if not isinstance(target, NecroSpiderTarget):
            raise TypeError(f"target is {type(target)}; expected NecroSpiderTarget")
    ...
    def setDbh(self, dbh) -> None:
        ...
        Args:
            dbh (NecroSpiderDb): database handle
        """
        self.__sfdb__ = dbh
    ...
    def tempStorage(self) -> dict:
        ...
        Note:
            Required for NecroSpider HX compatibility of modules.
    ...
    def notifyListeners(self, sfEvent) -> None:
        ...
        Args:
            sfEvent (NecroSpiderEvent): event

        Raises:
            TypeError: sfEvent argument was invalid type
        """

        from necrospider import NecroSpiderEvent

        if not isinstance(sfEvent, NecroSpiderEvent):
            raise TypeError(f"sfEvent is {type(sfEvent)}; expected NecroSpiderEvent")
    ...
    def threadWorker(self) -> None:
        try:
            ...
            self.setDbh(NecroSpiderDb(self.opts))
