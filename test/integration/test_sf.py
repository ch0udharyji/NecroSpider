...
class TestSf(unittest.TestCase):
    ...
    def test_no_args_should_print_arg_l_required(self):
        ...
        self.assertIn(b"NecroSpider requires -l <ip>:<port> to start the web server. Try --help for guidance.", out)
    ...
    def test_debug_arg_should_enable_and_print_debug_output(self):
        out, err, code = self.execute([sys.executable, "sf.py", "-d", "-m", "example module", "-s", "necrospider.net"])
    ...
    def test_quiet_arg_should_hide_debug_output(self):
        out, err, code = self.execute([sys.executable, "sf.py", "-q", "-m", "example module", "-s", "necrospider.net"])
    ...
    def test_run_scan_should_print_scan_result_and_exit(self):
        ...
        expected_output = [
            "Source,Type,Data",
            "NecroSpider UI,Internet Name,necrospider.net,necrospider.net\n",
            "NecroSpider UI,Domain Name,necrospider.net,necrospider.net\n",
            "sfp_countryname,Country Name,necrospider.net,United States\n",
        ]
