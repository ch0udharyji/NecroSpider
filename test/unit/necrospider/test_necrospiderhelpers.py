...
class TestNecroSpiderHelpers(unittest.TestCase):

    def test_data_path_should_return_a_string(self):
        data_path = NecroSpiderHelpers.dataPath()
        self.assertIsInstance(data_path, str)

    def test_cache_path_should_return_a_string(self):
        cache_path = NecroSpiderHelpers.cachePath()
        self.assertIsInstance(cache_path, str)

    def test_log_path_should_return_a_string(self):
        log_path = NecroSpiderHelpers.logPath()
        self.assertIsInstance(log_path, str)

    def test_target_type(self):
        target_type = NecroSpiderHelpers.targetTypeFromString("0.0.0.0")
        self.assertEqual('IP_ADDRESS', target_type)
        target_type = NecroSpiderHelpers.targetTypeFromString("noreply@necrospider.net")
        self.assertEqual('EMAILADDR', target_type)
        target_type = NecroSpiderHelpers.targetTypeFromString("0.0.0.0/0")
        self.assertEqual('NETBLOCK_OWNER', target_type)
        target_type = NecroSpiderHelpers.targetTypeFromString("+1234567890")
        self.assertEqual('PHONE_NUMBER', target_type)
        target_type = NecroSpiderHelpers.targetTypeFromString('"Human Name"')
        self.assertEqual('HUMAN_NAME', target_type)
        target_type = NecroSpiderHelpers.targetTypeFromString('"abc123"')
        self.assertEqual('USERNAME', target_type)
        target_type = NecroSpiderHelpers.targetTypeFromString("1234567890")
        self.assertEqual('BGP_AS_OWNER', target_type)
        target_type = NecroSpiderHelpers.targetTypeFromString("::1")
        self.assertEqual('IPV6_ADDRESS', target_type)
        target_type = NecroSpiderHelpers.targetTypeFromString("necrospider.net")
        self.assertEqual('INTERNET_NAME', target_type)
        target_type = NecroSpiderHelpers.targetTypeFromString("1HesYJSP1QqcyPEjnQ9vzBL1wujruNGe7R")
        self.assertEqual('BITCOIN_ADDRESS', target_type)

    def test_target_type_invalid_seed_should_return_none(self):
        target_type = NecroSpiderHelpers.targetTypeFromString(None)
        self.assertEqual(None, target_type)

        target_type = NecroSpiderHelpers.targetTypeFromString("")
        self.assertEqual(None, target_type)
    ...
    def test_urlRelativeToAbsolute_argument_url_relative_path_should_return_an_absolute_path(self):
        relative_url = NecroSpiderHelpers.urlRelativeToAbsolute('/somewhere/else/../../path?param=value#fragment')
    ...
    def test_url_base_dir_should_return_a_string(self):
        base_dir = NecroSpiderHelpers.urlBaseDir('http://localhost.local/path?param=value#fragment')
    ...
    def test_url_base_url_should_return_a_string(self):
        base_url = NecroSpiderHelpers.urlBaseUrl('http://localhost.local/path?param=value#fragment')
    ...
    def test_dictionaryWordsFromWordlists_should_return_a_set(self):
        words = NecroSpiderHelpers.dictionaryWordsFromWordlists()
    ...
    def test_dictionaryWordsFromWordlists_argument_wordlists_missing_wordlist_should_raise_IOError(self):
        with self.assertRaises(IOError):
            NecroSpiderHelpers.dictionaryWordsFromWordlists(['does not exist'])

    def test_humanNamesFromWordlists_should_return_a_set(self):
        names = NecroSpiderHelpers.humanNamesFromWordlists()
    ...
    def test_humanNamesFromWordlists_argument_wordlists_missing_wordlist_should_raise_IOError(self):
        with self.assertRaises(IOError):
            NecroSpiderHelpers.humanNamesFromWordlists(['does not exist'])

    def test_usernamesFromWordlists_should_return_a_set(self):
        users = NecroSpiderHelpers.usernamesFromWordlists()
    ...
    def test_usernamesFromWordlists_argument_wordlists_missing_wordlist_should_raise_IOError(self):
        with self.assertRaises(IOError):
            NecroSpiderHelpers.usernamesFromWordlists(['does not exist'])
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    NecroSpiderHelpers.buildGraphData(invalid_type)

    def test_buildGraphData_empty_data_should_raise_ValueError(self):
        with self.assertRaises(ValueError):
            NecroSpiderHelpers.buildGraphData([])
    ...
    def test_buildGraphData_should_return_a_set(self):
        graph_data = NecroSpiderHelpers.buildGraphData(
            [
    ...
    def test_buildGraphGexf_should_return_bytes(self):
        gexf = NecroSpiderHelpers.buildGraphGexf('test root', 'test title', [["test", "test", "test", "test", "test", "test", "test", "test", "test", "test", "test", "ENTITY", "test", "test", "test"]])
        self.assertIsInstance(gexf, bytes)

        self.assertEqual('TBD', 'TBD')

    def test_buildGraphJson_should_return_a_string(self):
        json = NecroSpiderHelpers.buildGraphJson('test root', [["test", "test", "test", "test", "test", "test", "test", "test", "test", "test", "test", "ENTITY", "test", "test", "test"]])
    ...
    def test_dataParentChildToTree_invalid_data_type_should_return_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    NecroSpiderHelpers.dataParentChildToTree(invalid_type)

    def test_dataParentChildToTree_empty_data_should_return_ValueError(self):
        with self.assertRaises(ValueError):
            NecroSpiderHelpers.dataParentChildToTree(dict())

    def test_dataParentChildToTree_should_return_dict(self):
        tree = NecroSpiderHelpers.dataParentChildToTree({"test": {"123": "456"}})
        self.assertIsInstance(tree, dict)

    def test_genScanInstanceId_should_return_a_string(self):
        scan_instance_id = NecroSpiderHelpers.genScanInstanceId()
    ...
    def test_extractLinksFromHtml_argument_data_not_containing_links_should_return_an_empty_dict(self):
        parse_links = NecroSpiderHelpers.extractLinksFromHtml('url', '<html>example html content</html>', 'domains')
    ...
    def test_extractLinksFromHtml_argument_data_containing_malformed_html_with_links_should_return_a_dict_of_urls(self):
        url = 'http://necrospider.net/'
        parse_links = NecroSpiderHelpers.extractLinksFromHtml(
            url,
            '<!DOCTYPE html><html lang="en-US"><meta charset="UTF-8" /><link rel="pingback" href="http://necrospider.net/xmlrpc.php">example html content<unclosed tag><a href="http://necrospider.net/path"></a><a href="/relative-path"></a></html>',
            'domains'
        )
        self.assertIsInstance(parse_links, dict)
        ...
        self.assertEqual(
            parse_links,
            {
                'http://necrospider.net/xmlrpc.php': {'source': url, 'original': 'http://necrospider.net/xmlrpc.php'},
                'http://necrospider.net/path': {'source': url, 'original': 'http://necrospider.net/path'},
                'http://necrospider.net/relative-path': {'source': url, 'original': '/relative-path'},
            }
        )
    ...
    def test_extractLinksFromHtml_invalid_url_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    NecroSpiderHelpers.extractLinksFromHtml(invalid_type, 'example html content', 'domains')
    ...
    def test_extractLinksFromHtml_invalid_data_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    NecroSpiderHelpers.extractLinksFromHtml("", invalid_type, 'domains')

    def test_extractLinksFromHtml_invalid_domains_should_return_a_dict(self):
        invalid_types = [None, "", bytes(), list(), dict()]
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                parse_links = NecroSpiderHelpers.extractLinksFromHtml('url', 'example html content', invalid_type)
                self.assertIsInstance(parse_links, dict)
    ...
    def test_extractHashesFromText_should_return_a_list(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                hashes = NecroSpiderHelpers.extractHashesFromText(invalid_type)
    ...
    def test_extractHashesFromText_argument_data_should_return_hahes(self):
        ...
        hashes = NecroSpiderHelpers.extractHashesFromText(f"necrospider{md5_hash}necrospider{sha1_hash}necrospider{sha256_hash}necrospider{sha512_hash}necrospider")
    ...
    def test_valid_email_should_return_a_boolean(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                valid_email = NecroSpiderHelpers.validEmail(invalid_type)
                self.assertIsInstance(valid_email, bool)
                self.assertFalse(valid_email)

        valid_email = NecroSpiderHelpers.validEmail('%@localhost.local')
        self.assertIsInstance(valid_email, bool)
        self.assertFalse(valid_email)
        ...
        valid_email = NecroSpiderHelpers.validEmail('root@localhost.local\n.com')
        self.assertIsInstance(valid_email, bool)
        self.assertFalse(valid_email)

        valid_email = NecroSpiderHelpers.validEmail('root@localhost')
        self.assertIsInstance(valid_email, bool)
        self.assertFalse(valid_email)
    ...
    def test_validPhoneNumber_should_return_a_boolean(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                valid_phone = NecroSpiderHelpers.validPhoneNumber(invalid_type)
                self.assertIsInstance(valid_phone, bool)
                self.assertFalse(valid_phone)

        valid_phone = NecroSpiderHelpers.validPhoneNumber('+1234567890')
        self.assertIsInstance(valid_phone, bool)
        self.assertFalse(valid_phone)

        valid_phone = NecroSpiderHelpers.validPhoneNumber('+12345678901234567890')
        self.assertIsInstance(valid_phone, bool)
        self.assertFalse(valid_phone)

        valid_phone = NecroSpiderHelpers.validPhoneNumber('+12345678901')
        self.assertIsInstance(valid_phone, bool)
    ...
    def test_validLEI_should_return_a_boolean(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                valid_phone = NecroSpiderHelpers.validLEI(invalid_type)
                self.assertIsInstance(valid_phone, bool)
                self.assertFalse(valid_phone)

        valid_lei = NecroSpiderHelpers.validLEI('7ZW8QJWVPR4P1J1KQYZZ')
        self.assertIsInstance(valid_lei, bool)
        self.assertFalse(valid_lei)

        valid_lei = NecroSpiderHelpers.validLEI('7ZW8QJWVPR4P1J1KQY45')
        self.assertIsInstance(valid_lei, bool)
        self.assertTrue(valid_lei)

    def test_countryCodes_should_return_a_dict(self):
        country_code_dict = NecroSpiderHelpers.countryCodes()
    ...
    def test_countryNameFromCountryCode_argument_countryCode_should_return_country_as_a_string(self):
        country_name = NecroSpiderHelpers.countryNameFromCountryCode('US')
    ...
    def test_countryNameFromTld_argument_tld_should_return_country_as_a_string(self):
        ...
        for tld in tlds:
            with self.subTest(tld=tld):
                country_name = NecroSpiderHelpers.countryNameFromTld(tld)
                self.assertIsInstance(country_name, str)
                self.assertEqual(country_name, "United States")

    def test_extractEmailsFromText_should_return_list_of_emails_from_string(self):
        emails = NecroSpiderHelpers.extractEmailsFromText("<html><body><p>From:noreply@necrospider.net</p><p>Subject:Hello user@necrospider.net, here's some text</p></body></html>")
        self.assertIsInstance(emails, list)
        self.assertIn('noreply@necrospider.net', emails)
        self.assertIn('user@necrospider.net', emails)

    def test_extractEmailsFromText_invalid_data_should_return_list(self):
        invalid_types = [None, "", bytes(), list(), dict()]
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                emails = NecroSpiderHelpers.extractEmailsFromText(invalid_type)
                self.assertIsInstance(emails, list)
    ...
    def test_extractPgpKeysFromText_should_return_list_of_pgp_keys_from_string(self):
        ...
        keys = NecroSpiderHelpers.extractPgpKeysFromText(f"<html><body><p>sample{key1}sample</p><p>sample{key2}sample</p></body></html>")
    ...
    def test_extractIbansFromText_should_return_a_list(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                ibans = NecroSpiderHelpers.extractIbansFromText(invalid_type)
        ...
        for iban in ibans:
            with self.subTest(iban=iban):
                extract_ibans = NecroSpiderHelpers.extractIbansFromText(iban)
    ...
    def test_extractCreditCardsFromText_should_return_a_list(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                cards = NecroSpiderHelpers.extractCreditCardsFromText(invalid_type)
                self.assertIsInstance(cards, list)

        cards = NecroSpiderHelpers.extractCreditCardsFromText("necrospider4111 1111 1111 1111necrospider")
        self.assertIsInstance(cards, list)
        self.assertEqual(["4111111111111111"], cards)

    def test_sslDerToPem_should_return_a_certificate_as_a_string(self):
        pem = NecroSpiderHelpers.sslDerToPem(
    ...
    def test_sslDerToPem_invalid_cert_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type), self.assertRaises(TypeError):
                NecroSpiderHelpers.sslDerToPem(invalid_type)

    def test_extractUrlsFromText_should_extract_urls_from_string(self):
        invalid_types = [None, "", bytes(), list(), dict()]
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                cards = NecroSpiderHelpers.extractUrlsFromText(invalid_type)
                self.assertIsInstance(cards, list)

        urls = NecroSpiderHelpers.extractUrlsFromText("abchttps://example.necrospider.net/path\rabchttp://example.necrospider.net:1337/path\rabc")
        self.assertIsInstance(urls, list)
        self.assertIn("https://example.necrospider.net/path", urls)
        self.assertIn("http://example.necrospider.net:1337/path", urls)

    def test_extractUrlsFromRobotsTxt_should_return_list(self):
        invalid_types = [None, "", bytes(), list(), dict()]
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                robots_txt = NecroSpiderHelpers.extractUrlsFromRobotsTxt(invalid_type)
                self.assertIsInstance(robots_txt, list)

        robots_txt = NecroSpiderHelpers.extractUrlsFromRobotsTxt("disallow:")
        self.assertIsInstance(robots_txt, list)
        self.assertFalse(robots_txt)

        robots_txt = NecroSpiderHelpers.extractUrlsFromRobotsTxt("disallow: /disallowed/path\n")
        self.assertIsInstance(robots_txt, list)
        self.assertIn("/disallowed/path", robots_txt)

    def test_sanitise_input(self):
        safe = NecroSpiderHelpers.sanitiseInput("example-string")
        ...
        safe = NecroSpiderHelpers.sanitiseInput("12")
        self.assertIsInstance(safe, bool)
        self.assertFalse(safe)
