...
from necrospider import NecroSpiderThreadPool


@pytest.mark.usefixtures
class TestNecroSpiderThreadPool(unittest.TestCase):
    """
    Test NecroSpider
    """
    ...
    def test_threadPool(self):
        ...
        # Example 1: using map()
        with NecroSpiderThreadPool(threads) as pool:
            map_results = sorted(
                list(pool.map(
                    callback,
                    iterable,
        ...
        pool = NecroSpiderThreadPool(threads)
        pool.start()
        for i in iterable2:
