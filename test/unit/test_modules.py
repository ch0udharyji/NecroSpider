...
class TestNecroSpiderModuleLoading(unittest.TestCase):
    """
    Test NecroSpider module loading
    """

    @staticmethod
    def load_modules(sf):
        mod_dir = os.path.dirname(os.path.abspath(__file__)) + '/../../modules/'
        return NecroSpiderHelpers.loadModulesAsDict(mod_dir, ['sfp_template.py'])
    ...
    def test_module_watched_events_are_valid(self):
        sf = NecroSpider(self.default_options)
        sf.dbh = NecroSpiderDb(self.default_options, True)
