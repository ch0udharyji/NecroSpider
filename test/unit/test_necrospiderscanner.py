# test_necrospiderscanner.py
import pytest
import unittest
import uuid

from sfscan import NecroSpiderScanner


@pytest.mark.usefixtures
class TestNecroSpiderScanner(unittest.TestCase):
    """
    Test NecroSpiderScanStatus
    """
    ...
    def test_init_argument_start_false_should_create_a_scan_without_starting_the_scan(self):
        ...
        sfscan = NecroSpiderScanner("example scan name", scan_id, "necrospider.net", "INTERNET_NAME", module_list, opts, start=False)
        self.assertIsInstance(sfscan, NecroSpiderScanner)
    ...
    def test_init_argument_start_true_with_no_valid_modules_should_set_scanstatus_to_failed(self):
        ...
        sfscan = NecroSpiderScanner("example scan name", scan_id, "necrospider.net", "INTERNET_NAME", module_list, opts, start=True)
        self.assertIsInstance(sfscan, NecroSpiderScanner)
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    NecroSpiderScanner(invalid_type, scan_id, "necrospider.net", "IP_ADDRESS", module_list, self.default_options, start=False)
    ...
    def test_init_argument_scanName_as_empty_string_should_raise_ValueError(self):
        ...
        with self.assertRaises(ValueError):
            NecroSpiderScanner("", scan_id, "necrospider.net", "IP_ADDRESS", module_list, self.default_options, start=False)
    ...
    def test_init_argument_scanId_of_invalid_type_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    NecroSpiderScanner("example scan name", invalid_type, "necrospider.net", "IP_ADDRESS", module_list, self.default_options, start=False)
    ...
    def test_init_argument_scanId_as_empty_string_should_raise_ValueError(self):
        ...
        with self.assertRaises(ValueError):
            NecroSpiderScanner("example scan name", scan_id, "necrospider.net", "IP_ADDRESS", module_list, self.default_options, start=False)
    ...
    def test_init_argument_targetValue_of_invalid_type_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    NecroSpiderScanner("example scan name", scan_id, invalid_type, "IP_ADDRESS", module_list, self.default_options, start=False)
    ...
    def test_init_argument_targetValue_as_empty_string_should_raise_ValueError(self):
        ...
        with self.assertRaises(ValueError):
            NecroSpiderScanner("example scan name", scan_id, "", "IP_ADDRESS", module_list, self.default_options, start=False)
    ...
    def test_init_argument_targetType_of_invalid_type_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    NecroSpiderScanner("example scan name", scan_id, "necrospider.net", invalid_type, module_list, self.default_options, start=False)
    ...
    def test_init_argument_targetType_invalid_value_should_raise_ValueError(self):
        ...
        target_type = ""
        with self.assertRaises(ValueError):
            NecroSpiderScanner("example scan name", scan_id, "necrospider.net", target_type, module_list, self.default_options, start=False)
    ...
    def test_init_argument_moduleList_of_invalid_type_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    NecroSpiderScanner("example scan name", scan_id, "necrospider.net", "IP_ADDRESS", invalid_type, self.default_options, start=False)
    ...
    def test_init_argument_globalOpts_of_invalid_type_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    NecroSpiderScanner("example scan name", scan_id, "necrospider.net", "IP_ADDRESS", module_list, invalid_type, start=False)
    ...
    def test_init_argument_globalOpts_as_empty_dict_should_raise_ValueError(self):
        ...
        with self.assertRaises(ValueError):
            NecroSpiderScanner("example scan name", scan_id, "necrospider.net", "IP_ADDRESS", module_list, dict(), start=False)
    ...
    def test_init_argument_globalOpts_proxy_invalid_proxy_type_should_raise_ValueError(self):
        ...
        with self.assertRaises(ValueError):
            NecroSpiderScanner("example scan name", scan_id, "necrospider.net", "IP_ADDRESS", module_list, opts, start=False)
    ...
    def test_attribute_scanId_should_return_scan_id_as_a_string(self):
        ...
        sfscan = NecroSpiderScanner("example scan name", scan_id, "necrospider.net", "IP_ADDRESS", module_list, opts, start=False)
        self.assertIsInstance(sfscan, NecroSpiderScanner)
    ...
    def test__setStatus_argument_status_of_invalid_type_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    sfscan._NecroSpiderScanner__setStatus(invalid_type)
    ...
    def test__setStatus_argument_status_with_blank_value_should_raise_ValueError(self):
        ...
        sfscan = NecroSpiderScanner("example scan name", scan_id, "necrospider.net", "IP_ADDRESS", module_list, opts, start=False)
        with self.assertRaises(ValueError):
            sfscan._NecroSpiderScanner__setStatus("example invalid scan status")
