...
from necrospider import NecroSpiderDb, NecroSpiderEvent, NecroSpiderPlugin, NecroSpiderTarget, NecroSpiderHelpers, NecroSpiderThreadPool, NecroSpiderCorrelator, logger


def startNecroSpiderScanner(loggingQueue, *args, **kwargs):
    logger.logWorkerSetup(loggingQueue)
    return NecroSpiderScanner(*args, **kwargs)


class NecroSpiderScanner():
    """NecroSpiderScanner object.
    ...
    def __init__(self, scanName: str, scanId: str, targetValue: str, targetType: str, moduleList: list, globalOpts: dict, start: bool = True) -> None:
        """Initialize NecroSpiderScanner object.
        ...
        self.__dbh = NecroSpiderDb(self.__config)
        ...
        self.__sf = NecroSpider(self.__config)
        ...
        if scanId:
            ...
            self.__scanId = NecroSpiderHelpers.genScanInstanceId()
        ...
        # Create our target
        try:
            self.__target = NecroSpiderTarget(self.__targetValue, self.__targetType)
        ...
        self.__sharedThreadPool = NecroSpiderThreadPool(threads=self.__config.get("_maxthreads", 3), name='sharedThreadPool')
    ...
    def __startScan(self) -> None:
        ...
        try:
            ...
            psMod = NecroSpiderPlugin()
            psMod.__name__ = "NecroSpider UI"
            ...
            rootEvent = NecroSpiderEvent("ROOT", self.__targetValue, "", None)
            psMod.notifyListeners(rootEvent)
            firstEvent = NecroSpiderEvent(self.__targetType, self.__targetValue,
                                         "NecroSpider UI", rootEvent)
            ...
            if self.__targetType == 'INTERNET_NAME' and self.__sf.isDomain(self.__targetValue, self.__config['_internettlds']):
                firstEvent = NecroSpiderEvent('DOMAIN_NAME', self.__targetValue, "NecroSpider UI", rootEvent)
                psMod.notifyListeners(firstEvent)
    ...
    def runCorrelations(self) -> None:
        ...
        corr = NecroSpiderCorrelator(self.__dbh, ruleset, self.__scanId)
