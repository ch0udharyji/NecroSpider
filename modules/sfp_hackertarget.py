...
# Purpose:      NecroSpider plug-in to search HackerTarget.com for hosts sharing
...
class sfp_hackertarget(NecroSpiderPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == 'DOMAIN_NAME_PARENT':
            ...
            evt = NecroSpiderEvent('RAW_DNS_RECORDS', "\n".join(records), self.__name__, event)
                ...
                for host in set(hosts):
                    ...
                    if self.sf.isDomain(host, self.opts['_internettlds']):
                        if evt_type.startswith('AFFILIATE'):
                            evt = NecroSpiderEvent('AFFILIATE_DOMAIN_NAME', host, self.__name__, event)
                            self.notifyListeners(evt)
                        else:
                            evt = NecroSpiderEvent('DOMAIN_NAME', host, self.__name__, event)
                            self.notifyListeners(evt)
        ...
        for ip in qrylist:
            ...
            for h in hosts:
                ...
                if h not in myres and h != ip:
                    ...
                    if self.cohostcount < self.opts['maxcohost']:
                        ...
                        # link to the co-host.
                        if eventName == "NETBLOCK_OWNER":
                            ipe = NecroSpiderEvent("IP_ADDRESS", ip, self.__name__, event)
                            self.notifyListeners(ipe)
                            evt = NecroSpiderEvent("CO_HOSTED_SITE", h.lower(), self.__name__, ipe)
                            self.notifyListeners(evt)
                        else:
                            evt = NecroSpiderEvent("CO_HOSTED_SITE", h.lower(), self.__name__, event)
                            self.notifyListeners(evt)
            ...
            if eventName == 'NETBLOCK_OWNER':
                pevent = NecroSpiderEvent("IP_ADDRESS", ip, self.__name__, event)
            ...
            if self.opts.get('http_headers', True):
                ...
                if http_headers is not None:
                    e = NecroSpiderEvent('WEBSERVER_HTTPHEADERS', json.dumps(http_headers), self.__name__, pevent)
                    e.actualSource = ip
