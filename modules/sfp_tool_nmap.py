...
# -------------------------------------------------------------------------------
# Name:         sfp_tool_nmap
# Purpose:      NecroSpider plug-in for using nmap to perform OS fingerprinting.
#
...
class sfp_tool_nmap(NecroSpiderPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "IP_ADDRESS":
            try:
                ...
                if opsys:
                    evt = NecroSpiderEvent("OPERATING_SYSTEM", opsys, self.__name__, event)
            ...
            try:
                ...
                for line in content.split('\n'):
                    ...
                    if opsys and currentIp:
                        ipevent = NecroSpiderEvent("IP_ADDRESS", currentIp, self.__name__, event)
                        self.notifyListeners(ipevent)

                        evt = NecroSpiderEvent("OPERATING_SYSTEM", opsys, self.__name__, ipevent)
                        self.notifyListeners(evt)
                        currentIp = None
            except Exception as e:
