...
class sfp_reversewhois(NecroSpiderPlugin):
    ...
    def handleEvent(self, event):
        ...
        for domain in set(domains):
            ...
            if not self.getTarget().matches(domain, includeChildren=False):
                e = NecroSpiderEvent("AFFILIATE_INTERNET_NAME", domain, self.__name__, event)
        ...
        for registrar in set(registrars):
            e = NecroSpiderEvent("DOMAIN_REGISTRAR", registrar, self.__name__, event)
            self.notifyListeners(e)
