...
# Purpose:      NecroSpider plug-in for performing a basic TCP port scan of IP
...
class sfp_portscan_tcp(NecroSpiderPlugin):
    ...
    def sendEvent(self, resArray, srcEvent):
        for cp in resArray:
            ...
            evt = NecroSpiderEvent("TCP_PORT_OPEN", cp, self.__name__, srcEvent)
            self.notifyListeners(evt)

            if resArray[cp] is not True:
                banner = str(resArray[cp], 'utf-8', errors='replace')
                bevt = NecroSpiderEvent("TCP_PORT_OPEN_BANNER", banner, self.__name__, evt)
