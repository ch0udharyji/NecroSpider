...
class sfp_builtwith(NecroSpiderPlugin):
            ...
            if "Meta" in data:
                if data['Meta'].get("Names", []):
                    for nb in data['Meta']['Names']:
                        e = NecroSpiderEvent("RAW_RIR_DATA", "Possible full name: " + nb['Name'],
                                            self.__name__, event)
                        self.notifyListeners(e)
                        if nb.get('Email', None):
                            if NecroSpiderHelpers.validEmail(nb['Email']):
                                if nb['Email'].split("@")[0] in self.opts['_genericusers'].split(","):
                                    evttype = "EMAILADDR_GENERIC"
                                else:
                                    evttype = "EMAILADDR"
                                e = NecroSpiderEvent(evttype, nb['Email'],
                                                    self.__name__, event)
                                self.notifyListeners(e)
                ...
                if data['Meta'].get("Emails", []):
                    for email in data['Meta']['Emails']:
                        if NecroSpiderHelpers.validEmail(email):
                            ...
                            e = NecroSpiderEvent(evttype, email,
                                                self.__name__, event)
                            self.notifyListeners(e)
                ...
                if data['Meta'].get("Telephones", []):
                    for phone in data['Meta']['Telephones']:
                        ...
                        e = NecroSpiderEvent("PHONE_NUMBER", phone, self.__name__, event)
                        self.notifyListeners(e)
            ...
            if "Paths" in data.get("Result", []):
                for p in data["Result"]['Paths']:
                    if p.get("SubDomain", ""):
                        ...
                        ev = NecroSpiderEvent("INTERNET_NAME", h, self.__name__, event)
                        self.notifyListeners(ev)
                        if self.sf.isDomain(h, self.opts['_internettlds']):
                            ev = NecroSpiderEvent("DOMAIN_NAME", h, self.__name__, event)
                            self.notifyListeners(ev)
                    else:
                        ev = None
                    ...
                    for t in p.get("Technologies", []):
                        ...
                        if t.get("LastDetected", 0) < agelimit:
                            ...
                            continue
                        e = NecroSpiderEvent("WEBSERVER_TECHNOLOGY", t["Name"],
                                            self.__name__, src)
                        self.notifyListeners(e)
        ...
        for r in data:
            ...
            for i in r['Identifiers']:
                ...
                if i['Type'] == "ip":
                    ...
                    e = NecroSpiderEvent(evttype, val, self.__name__, event)
                    self.notifyListeners(e)
                    continue

                # Related through shared analytics ID
                txt = i['Type'] + ": " + str(i['Value'])
                e = NecroSpiderEvent("WEB_ANALYTICS_ID", txt, self.__name__, event)
                self.notifyListeners(e)
                ...
                if i['Matches']:
                    for m in i['Matches']:
                        if "Domain" not in m:
                            continue
                        evt = NecroSpiderEvent("AFFILIATE_INTERNET_NAME", m['Domain'], self.__name__, e)
                        self.notifyListeners(evt)

                        if self.sf.isDomain(m['Domain'], self.opts['_internettlds']):
                            evt = NecroSpiderEvent("AFFILIATE_DOMAIN_NAME", m['Domain'], self.__name__, e)
                            self.notifyListeners(evt)
