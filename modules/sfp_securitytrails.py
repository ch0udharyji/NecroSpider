...
class sfp_securitytrails(NecroSpiderPlugin):
                ...
                for r in rec:
                    if "host_provider" in r:
                        ...
                        for dat in r['host_provider']:
                            if dat in hosters:
                                continue
                            e = NecroSpiderEvent("PROVIDER_HOSTING", dat,
                                                self.__name__, event)
                    ...
                    if "hostname" in r:
                        ...
                        e = NecroSpiderEvent("CO_HOSTED_SITE", h, self.__name__, event)
                        self.notifyListeners(e)
                        self.cohostcount += 1
                ...
                for r in rec:
                    if "hostname" in r:
                        ...
                        if h.lower() not in myres:
                            ...
                            continue
                        e = NecroSpiderEvent("AFFILIATE_INTERNET_NAME", h, self.__name__, event)
                        self.notifyListeners(e)

                        if self.sf.isDomain(h, self.opts['_internettlds']):
                            evt = NecroSpiderEvent("AFFILIATE_DOMAIN_NAME", h, self.__name__, event)
            ...
            if rec is not None:
                for h in rec:
                    ...
                    if h.lower() not in myres:
                        ...
                        continue
                    e = NecroSpiderEvent("INTERNET_NAME", h + "." + domain,
                                        self.__name__, event)
                    self.notifyListeners(e)
