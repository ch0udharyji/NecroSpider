...
class sfp_leakix(NecroSpiderPlugin):
        ...
        if eventName in ["IP_ADDRESS", "DOMAIN_NAME"]:
                ...
                for service in services:
                    ...
                    if hostname and eventName == "DOMAIN_NAME" and self.getTarget().matches(hostname) and hostname not in hosts:
                        if self.opts["verify"] and not self.sf.resolveHost(hostname) and not self.sf.resolveHost6(hostname):
                            ...
                            evt = NecroSpiderEvent("INTERNET_NAME_UNRESOLVED", hostname, self.__name__, event)
                        else:
                            evt = NecroSpiderEvent("INTERNET_NAME", hostname, self.__name__, event)
                        self.notifyListeners(evt)
                        src = evt
                        hosts.append(hostname)
                    ip = service.get('ip')
                    if ip and eventName != "IP_ADDRESS" and self.sf.validIP(ip) and ip not in ips:
                        evt = NecroSpiderEvent("IP_ADDRESS", ip, self.__name__, src)
                        self.notifyListeners(evt)
                        ips.append(ip)
                        ipevt = evt
                    port = service.get('port')
                    if port and ip + ":" + port not in ports:
                        evt = NecroSpiderEvent("TCP_PORT_OPEN", ip + ':' + port, self.__name__, src)
                        self.notifyListeners(evt)
                        ...
                        if servers:
                            for server in servers:
                                if server and server not in banners:
                                    evt = NecroSpiderEvent('WEBSERVER_BANNER', server, self.__name__, src)
                                    self.notifyListeners(evt)
                    ...
                    if geoip:
                        ...
                        if location:
                            if ip and self.sf.validIP(ip) and ipevt:
                                ...
                                evt = NecroSpiderEvent("GEOINFO", location, self.__name__, ipevt)
                                self.notifyListeners(evt)
                    ...
                    if software:
                        ...
                        if software_version and software_version not in softwares:
                            evt = NecroSpiderEvent("SOFTWARE_USED", software_version, self.__name__, src)
                            self.notifyListeners(evt)
                            softwares.append(software_version)

                        os = software.get('os')
                        if os and os not in oses:
                            evt = NecroSpiderEvent('OPERATING_SYSTEM', os, self.__name__, src)
                            self.notifyListeners(evt)
                            oses.append(os)
            ...
            if leaks:
                for leak in leaks:
                    ...
                    if leak_data:
                        evt = NecroSpiderEvent("LEAKSITE_CONTENT", leak_data, self.__name__, event)
                        self.notifyListeners(evt)
