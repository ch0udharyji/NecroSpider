...
class sfp_searchcode(NecroSpiderPlugin):
    ...
    def handleEvent(self, event):
        ...
        while page < max_pages:
            ...
            emails = NecroSpiderHelpers.extractEmailsFromText(str(results))
            for email in emails:
                ...
                evt = NecroSpiderEvent(evt_type, email, self.__name__, event)
            ...
            for result in results:
                ...
                if lines:
                    for line in lines:
                        links.update(NecroSpiderHelpers.extractUrlsFromText(lines[line]))
            ...
            for link in links:
                ...
                if self.opts['dns_resolve'] and not self.sf.resolveHost(host) and not self.sf.resolveHost6(host):
                    ...
                    evt = NecroSpiderEvent("INTERNET_NAME_UNRESOLVED", host, self.__name__, event)
                    self.notifyListeners(evt)
                else:
            ...
            for result in results:
                ...
                evt = NecroSpiderEvent('PUBLIC_CODE_REPO', repo_data, self.__name__, event)
                self.notifyListeners(evt)

                evt = NecroSpiderEvent('RAW_RIR_DATA', json.dumps(result), self.__name__, event)
                self.notifyListeners(evt)
