...
# Purpose:      NecroSpider plug-in for identifying potential Digital Ocean spaces
...
class sfp_digitaloceanspace(NecroSpiderPlugin):
    ...
    def checkSite(self, url):
        res = self.sf.fetchUrl(url, timeout=10, useragent="NecroSpider", noLog=True)
    ...
    def handleEvent(self, event):
        ...
        if eventName == "LINKED_URL_EXTERNAL":
            if ".digitaloceanspaces.com" in eventData:
                ...
                evt = NecroSpiderEvent("CLOUD_STORAGE_BUCKET", b, self.__name__, event)
        ...
        for b in ret:
            bucket = b.split(":")
            evt = NecroSpiderEvent("CLOUD_STORAGE_BUCKET", bucket[0] + ":" + bucket[1], self.__name__, event)
            self.notifyListeners(evt)
            if bucket[2] != "0":
                evt = NecroSpiderEvent("CLOUD_STORAGE_BUCKET_OPEN", bucket[0] + ":" + bucket[1] + ": " + bucket[2] + " files found.",
                                      self.__name__, evt)
                self.notifyListeners(evt)
