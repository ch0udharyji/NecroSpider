...
class sfp_metadefender(NecroSpiderPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == 'IP_ADDRESS':
            ...
            for m in sources:
                ...
                evt = NecroSpiderEvent('MALICIOUS_IPADDR', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
                evt = NecroSpiderEvent('BLACKLISTED_IPADDR', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
        ...
        if eventName == 'INTERNET_NAME':
            ...
            for m in sources:
                ...
                if m['assessment'] == "trustworthy":
                    continue
                provider = m.get('provider')
                evt = NecroSpiderEvent('MALICIOUS_INTERNET_NAME', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
                evt = NecroSpiderEvent('BLACKLISTED_INTERNET_NAME', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
