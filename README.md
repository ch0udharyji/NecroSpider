<div align="center">
  <a href="https://ch0udharyji.com">
    <img src="./necrospider/static/img/necrospider-header-dark.png" alt="NecroSpider Logo">
  </a>
  ...
  <p>
    <b>NecroSpider</b> is an advanced open source intelligence (OSINT) automation tool.<br>
    It integrates with over 200 data sources and utilizes a range of methods for data analysis,<br>
    making complex intelligence gathering easy to navigate.
  </p>

  <p>
    <i>This project is a heavily modified and modernized fork of <b>SpiderFoot v4.0.0</b>,<br>
    featuring a complete aesthetic overhaul, an interactive Rust-based CLI, and a stunning dark-mode interface.</i>
  </p>
</div>

---

## What's new in NecroSpider vs SpiderFoot v4.0.0?

If you are coming from the original SpiderFoot project, **NecroSpider** introduces several major upgrades:
...
## Tech Used
- **Backend:** Python 3.7+
- **Frontend UI:** HTML5, Custom Vanilla CSS, JavaScript, D3.js (for graphs), Bootstrap
- **Launcher / CLI:** Rust (using `crossterm` and `inquire` for the TUI)
- **Database:** SQLite backend for custom querying
- **Configuration:** YAML-configurable correlation engine
...
Simply type the command in your terminal from anywhere:
```bash
necrospider-cli
```

*(Note: During the very first run, the CLI will automatically `git clone` the NecroSpider repository into `~/.necrospider` on your machine so it can manage its files seamlessly.)*
...
NecroSpider can be used offensively (e.g. in a red team exercise or penetration test) for reconnaissance of your target or defensively to gather information about what you or your organisation might have exposed over the Internet.

You can target the following entities in a NecroSpider scan:
- IP address, Subnets (CIDR), ASN
- Domain/sub-domain name, Hostname
- E-mail address, Phone number
- Username, Person's name
- Bitcoin address

NecroSpider's 200+ modules feed each other in a publisher/subscriber model to ensure maximum data extraction. When a scan starts, it cascades through the modules—for example, a Domain Name target might resolve to an IP Address, which then triggers the Port Scanner module, which then triggers the Web Scraping module, and so on.

---

## Contributors

NecroSpider was modified with passion by [ch0udharyji](https://ch0udharyji.com). 

This project is built upon the incredible foundation of **SpiderFoot v4.0.0**. Please see [CONTRIBUTORS.md](CONTRIBUTORS.md) for full attribution to the original creator (`@smicallef`) and the open-source community.

---

## Modules / Integrations

NecroSpider has over 200 modules, most of which *don't require API keys*, and many of those that do require API keys *have a free tier*.
